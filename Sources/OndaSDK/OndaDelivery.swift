...
public enum OndaDelivery {
    ...
    public static func reportDelivered(messageId: String, appGroup: String?,
        ...
        guard let resolved = SharedConfig.resolve(appGroup: appGroup) else {
            OndaLog.warn("NSE 도달 전송 불가 — app group 설정 미러링 없음 (코어 initialize 전이거나 appGroup 불일치)")
            completion()
            return
        }
        guard let item = buildEvent(messageId: messageId, resolved: resolved, now: Date()) else {
            OndaLog.warn("NSE 도달 전송 건너뜀 — 유효한 anon_id(UUID)/external_id 없음 (message_id=\(messageId))")
            completion()
            return
        }
        ...
        network.sendTrack([item]) { ok in
            if !ok { OndaLog.warn("NSE 도달 전송 실패 (message_id=\(messageId))") }
            completion()
        }
    }
