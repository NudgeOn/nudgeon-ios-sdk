...
public struct PushPayload: Equatable {
    ...
    public static func parse(_ userInfo: [AnyHashable: Any]) -> PushPayload? {
        ...
        return PushPayload(
            messageId: messageId,
            campaignId: stringValue(nudgeon["campaign_id"]),
            journeyId: stringValue(nudgeon["journey_id"]),
            title: title,
            body: body,
...
public struct SubscriptionState: Equatable {
    public let serviceOptIn: Bool     // NudgeOn.setPushSubscription로 제어하는 서비스 수준 수신 동의
