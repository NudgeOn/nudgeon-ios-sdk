...
final class NudgeOnCore {
    ...
    convenience init(config: NudgeOnConfig) {
    ...
    func setUserAttributes(_ attrs: [String: NudgeOnValue]) {
        work.async { [self] in
            guard let ext = identity.externalId else {
                NudgeOnLog.warn("setUserAttributes: identify 이전 호출 — 무시 (익명 속성은 후속)")
                return
            }
