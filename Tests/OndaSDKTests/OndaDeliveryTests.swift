...
/// 서버 수집 스키마(onda-platform apps/api/src/ingestion/schemas.ts)의 Swift 미러.
...
final class OndaDeliveryTests: XCTestCase {
    ...
    private func body(for item: EventQueue.Item, device: String? = nil) -> [String: Any] {
        let config = OndaConfig(sdkKey: "pk_test", apiHost: URL(string: "https://ingest.example.com")!)
        return Network(config: config, deviceId: device ?? deviceId).trackBody([item])
    }

    func testAnonOnlyPayloadSatisfiesServerSchema() throws {
        let item = try XCTUnwrap(OndaDelivery.buildEvent(messageId: "m-1", resolved: resolved(anon: anonId, ext: nil), now: Date()))
        let b = body(for: item)
    ...
    func testIdentifiedPayloadCarriesExternalId() throws {
        let item = try XCTUnwrap(OndaDelivery.buildEvent(messageId: "m-2", resolved: resolved(anon: anonId, ext: "user-123"), now: Date()))
        let b = body(for: item)
    ...
    func testExternalOnlyPayloadOmitsAnonKey() throws {
        ...
        let item = try XCTUnwrap(OndaDelivery.buildEvent(messageId: "m-3", resolved: resolved(anon: nil, ext: "user-1"), now: Date()))
        let b = body(for: item)
        XCTAssertEqual(ServerTrackSchema.validateBody(b), [])
        let e = (b["batch"] as! [[String: Any]])[0]
    ...
    func testEmptyAnonAndNoExternalIsSkipped() {
        XCTAssertNil(OndaDelivery.buildEvent(messageId: "m-4", resolved: resolved(anon: "", ext: nil), now: Date()),
                     "빈 anon_id·external_id 없음 → 전송 건너뜀 (400 예방)")
        XCTAssertNil(OndaDelivery.buildEvent(messageId: "m-4", resolved: resolved(anon: nil, ext: nil), now: Date()),
                     "식별자 미러링 없음 → 전송 건너뜀")
        XCTAssertNil(OndaDelivery.buildEvent(messageId: "m-4", resolved: resolved(anon: "not-a-uuid", ext: ""), now: Date()),
                     "UUID 아닌 anon_id·빈 external_id → 전송 건너뜀")
    }

    func testInvalidDeviceIdIsSkipped() {
        XCTAssertNil(OndaDelivery.buildEvent(messageId: "m-5", resolved: resolved(anon: anonId, ext: nil, device: "dev-1"), now: Date()),
    ...
    func testSharedConfigRoundTripsIdentityAndClearsOnReset() throws {
        let group = "onda.test.group.\(UUID().uuidString)"
        let config = OndaConfig(sdkKey: "pk_test", apiHost: URL(string: "https://ingest.example.com")!, appGroup: group)
        ...
        let item = try XCTUnwrap(OndaDelivery.buildEvent(messageId: "m-6", resolved: r, now: Date()))
