...
final class IdentityTests: XCTestCase {
    private func freshDefaults() -> UserDefaults {
        let d = UserDefaults(suiteName: "nudgeon.test.\(UUID().uuidString)")!
        return d
    }
...
final class PushPayloadTests: XCTestCase {
    ...
    func testReturnsNilForNonNudgeOnMessage() {
        // nudgeon.message_id 없음 → 타 SDK 메시지로 간주, nil (공존).
...
final class PushManagerTests: XCTestCase {
    private func make() -> (PushManager, UserDefaults) {
        let d = UserDefaults(suiteName: "nudgeon.push.test.\(UUID().uuidString)")!
        let cfg = NudgeOnConfig(sdkKey: "pk", apiHost: URL(string: "https://ingest.example.com")!)
