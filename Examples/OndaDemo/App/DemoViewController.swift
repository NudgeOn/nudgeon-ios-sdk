...
final class DemoViewController: UIViewController {
    ...
    private func configureLayout() {
        ...
        let trackButton = makeButton("Track product_viewed", identifier: "track-button") {
            Onda.track("product_viewed", properties: ["product_id": "P-1", "price": 12_900])
        ...
        let checkoutButton = makeButton("Track checkout + flush", identifier: "flush-button") {
            Onda.track("checkout_started", properties: ["cart_value": 39_000])
            Onda.flush()
        ...
        let attributesButton = makeButton("Set user attributes", identifier: "attributes-button") {
            Onda.setUserAttributes(["vip_level": .number(3), "nickname": .string("sample")])
        ...
        let deepLinkButton = makeButton("Open ondademo://product/P-1", identifier: "deep-link-button") { [weak self] in
    ...
    private func configureActions() {
        serviceOptInSwitch.addAction(UIAction { action in
            ...
            Onda.setPushSubscription(toggle.isOn)
            DemoActivityCenter.shared.record(
    ...
    private func identifyUser() {
        ...
        Onda.identify(externalId: externalID)
        DemoActivityCenter.shared.record(
    ...
    private func requestPushPermission(provisional: Bool = false) {
        ...
        Task { [weak self] in
            let result = await Onda.registerForPush(provisional: provisional)
            DemoActivityCenter.shared.record(
    ...
    private func refreshPushStateAsync() async {
        let state = await Onda.getPushSubscription()
    ...
    private func refreshIdentity() {
        identityLabel.text = [
            "device_id: \(Onda.getDeviceId() ?? "-")",
            "anon_id: \(Onda.getAnonId() ?? "-")",
        ].joined(separator: "\n")
    }
    ...
    private func openDemoDeepLink() {
        guard let url = URL(string: "ondademo://product/P-1") else { return }
